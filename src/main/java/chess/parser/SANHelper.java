...
public class SANHelper implements PieceColor, Piece {
    ...
    public static String getSanFromLAN(String lan, ChessGame chessGame) {
        if (lan == null || lan.isEmpty()) return null;
        int movesCount = chessGame.getMoveList().size();
        MoveFactory moveFactory = new MoveFactory(new PossibleMovesProviderImpl(chessGame));
        String[] moves = lan.split(" ");
        for (String s : moves) {
            char[] chars = s.trim().toCharArray();
            if (chars.length >= 4) {
                Move move = moveFactory.getMove(chessGame.getBoard(), chars[0] - 'a', chars[1] - '1', chars[2] - 'a', chars[3] - '1');
                if (chars.length == 5) {
                    ...
                    chessGame.getBoard()[move.getToX()][move.getToY()] = move.getPromotion() + move.getColor();
                }
        ...
        for (int i = movesCount; i < chessGame.getMoveList().size(); i++) {
            Move m = chessGame.getMoveList().get(i);
